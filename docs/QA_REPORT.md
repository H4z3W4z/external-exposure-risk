...
Follow-up at 13:07 UTC: a deployment regression fix raised the suite to 95 passing tests, and a private hosted build plus live Shodan acceptance run passed; see [LIVE_ACCEPTANCE.md](LIVE_ACCEPTANCE.md). The earlier machine-readable report above remains a historical snapshot. Known limitations: representative live portfolios and CVE-bearing live observations have not been validated. Commercial licensing and final costs remain unresolved. One known moderate `stream-json` advisory remains accepted for the currently reviewed, unaffected import path; see [BUILD_NOTES.md](../BUILD_NOTES.md). QA fails on new advisory identities or severities. Deployment API behavior is mock-tested and the archive source was verified in a hosted build; dry run performs no hosted mutation.
