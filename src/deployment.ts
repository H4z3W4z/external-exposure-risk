export interface DeploymentPlan {
  ...
  gitRepoUrl: string; buildTag: string; applyEnvVarsToBuild: false;
...
export function deploymentPlan(repository: string, commit: string, name = 'external-exposure-risk', version = '0.1'): DeploymentPlan {
  ...
  return { name, isPublic: false, version, repository: repo, commit, gitRepoUrl: `${repo}.git#${commit}`,
...
export interface DeploymentApi {
  ...
  getVersion(id: string, version: string): Promise<{ gitRepoUrl?: string; sourceType?: string } | undefined>;
...
export async function applyDeployment(plan: DeploymentPlan, api: DeploymentApi) {
  ...
  if (!actor) actor = { ...(await api.createActor(plan)), isPublic: false };
    ...
    if (version && (version.sourceType !== 'GIT_REPO' || version.gitRepoUrl?.split('#')[0].replace(/\.git$/, '') !== plan.repository)) throw new Error('Existing version has a different source; refusing to overwrite it.');
